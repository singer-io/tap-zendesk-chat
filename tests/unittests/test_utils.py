...
class BaseMetadata:
    ...
    metadata = [
        {
            "breadcrumb": [],
            "metadata": {"valid-replication-keys": [], "table-key-properties": ["id"], "selected": True},
        },
        {"breadcrumb": ["properties", "create_date"], "metadata": {"inclusion": "available"}},
    ]


class Departments(BaseMetadata):
    """Class for Departments stream inherits BaseMetadata class."""
...
class Account(BaseMetadata):
    """Class for Account stream inherits BaseMetadata class."""
...
class Bans:
    """Class for Bans stream has its own metadata attribute."""
    ...
    metadata = [
        {
            "breadcrumb": [],
            "metadata": {"valid-replication-keys": [], "table-key-properties": ["id"], "selected": False},
        },
        {"breadcrumb": ["properties", "create_date"], "metadata": {"inclusion": "available"}},
    ]
...
class TestMetadataFunctions(unittest.TestCase):
    """Used to test metadata functions defined in tap_zendesk_chat/__init__.py
    file."""
    ...
    def test_load_schema(self):
        """tests load_schema fn in tap_zendesk_chat/__init__.py file checks if
        length of properties attr equals with size of properties in loaded
        schema using load_schema fn."""
        for stream in self.POSITIVE_TEST_STREAMS:
            self.assertEqual(len(stream.properties), len(utils.load_schema(stream.stream)["properties"]))
